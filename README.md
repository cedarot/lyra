...
If a provider repeatedly challenges a Playwright-launched browser, start Chrome with a separate profile and CDP enabled, for example `google-chrome --remote-debugging-port=9222 --user-data-dir=/tmp/lyra-browser-profile`. Lyra automatically detects the conventional local endpoint `http://127.0.0.1:9222` and reuses that browser context; `--browser-endpoint` is only needed for a different endpoint. Lyra connects to the existing browser context without closing it; JSON-only requests do not create a blank tab, and any Lyra-created page is opened lazily and closed after use. The browser owns its session and the user remains responsible for completing site verification.
